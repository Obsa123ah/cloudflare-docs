...
A primary use case for Dynamic Worker Loaders is [Code Mode](/agents/api-reference/code-mode/) in the [Agents SDK](/agents/). Code Mode converts [MCP](/agents/model-context-protocol/) tool schemas into a TypeScript API, asks the LLM to generate code that calls the API, and executes that code in a sandboxed isolate using the Worker Loader. This allows AI agents to chain multiple tool calls in a single execution, reducing round-trips through the model while keeping generated code fully isolated.

Refer to the [Code Mode documentation](/agents/api-reference/code-mode/) to learn more.
